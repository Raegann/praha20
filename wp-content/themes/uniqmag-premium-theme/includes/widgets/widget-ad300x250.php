...
class tz_ad300_widget extends WP_Widget {
	...
	function TZ_Ad300_Widget() {
		...
		/* Widget settings */
		$widget_ops = array( 'classname' => 'tz_ad300_widget', 'description' => esc_html__('A widget that allows the display and configuration of of a single 300x250 Banner.', 'uniqmag') );

		/* Widget control settings */
		$control_ops = array( 'width' => 300, 'height' => 250, 'id_base' => 'tz_ad300_widget' );

		/* Create the widget */
		parent::__construct ( 'tz_ad300_widget', esc_html__(THEME_FULL_NAME.' Custom 300x250 Ad', 'uniqmag'), $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		$contactID = uniqmag_different_themes_get_page("contact");
		...
		echo '<div class="cs-widget_banner_300x250">';
			echo '<div class="cs-banner">';
				...
				if($contactID) {
					//echo '<a href="'.esc_url(get_page_link($contactID[0])).'" class="banner-meta">'.esc_html__("Contact us about advert spaces",'uniqmag').'<i class="fa fa-angle-double-up"></i></a>';
				}
	...
	function form( $instance ) {
		...
		'subtitle' => '',
		'subtitle_link' => '',
		'ad' => get_template_directory_uri()."/images/3.jpg",
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'title' )); ?>"><?php esc_html_e('Title:','uniqmag') ?></label>
			<input class="widefat" id="<?php echo esc_attr($this->get_field_id( 'title' )); ?>" name="<?php echo esc_attr($this->get_field_name( 'title' )); ?>" value="<?php echo esc_attr($instance['title']); ?>" />
		</p>
		<!-- Ad image url: Text Input -->
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'ad' )); ?>"><?php esc_html_e('Ad image url:','uniqmag') ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'link' )); ?>"><?php esc_html_e('Ad link url:','uniqmag') ?></label>

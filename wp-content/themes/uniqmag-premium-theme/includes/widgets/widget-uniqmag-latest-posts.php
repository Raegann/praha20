<?php
add_action('widgets_init', create_function('', 'return register_widget("different_themes_cat_posts");'));

class different_themes_cat_posts extends WP_Widget {
	function different_themes_cat_posts() {
		 parent::__construct (false, $name = THEME_FULL_NAME.' '.esc_html__("Latests News",'uniqmag'));	
	}

	function form($instance) {
		/* Set up some default widget settings. */
		$defaults = array(
			'title' => esc_html__("Latests News",'uniqmag'),
			'subtitle' => '',
			'cat' => '',
			'style' => 'list',
			'count' => '3',
        ...
        ?>
			...
			<p><label for="<?php echo esc_attr($this->get_field_id('count')); ?>"><?php esc_html_e('Post count:','uniqmag');?> <input class="widefat" id="<?php echo esc_attr($this->get_field_id('count')); ?>" name="<?php echo esc_attr($this->get_field_name('count')); ?>" type="number" value="<?php echo esc_attr($count); ?>" /></label></p>
